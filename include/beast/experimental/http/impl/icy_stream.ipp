...
#ifndef BEAST_CORE_IMPL_ICY_STREAM_IPP
#define BEAST_CORE_IMPL_ICY_STREAM_IPP
...
buffer_shift(MutableBuffers const& out, ConstBuffers const& in)
{
    ...
    auto in_pos  = asio::buffer_sequence_end(in);
    auto out_pos = asio::buffer_sequence_end(out);
    auto const in_begin  = asio::buffer_sequence_begin(in);
    auto const out_begin = asio::buffer_sequence_begin(out);
    BOOST_ASSERT(buffer_size(in) == buffer_size(out));
    if(in_pos == in_begin || out_pos == out_begin)
        return;
    asio::const_buffer cb{*--in_pos};
    asio::mutable_buffer mb{*--out_pos};
    for(;;)
    {
        if(mb.size() >= cb.size())
        {
    ...
    std::size_t bytes_transferred)
{
    ...
    using asio::buffer_size;
    using iterator = asio::buffers_iterator<
            ...
            goto upcall;
        {
            ...
            if(d.b.size() + 5 > d.b.max_size())
            {
                d.s.copy_ = static_cast<unsigned char>(
                    n + 5 - d.b.max_size());
                std::copy(
                    asio::buffers_begin(d.b.value()) + n - d.s.copy_,
                    asio::buffers_begin(d.b.value()) + n,
                    d.s.buf_);
                n = d.b.max_size() - 5;
            }
            {
...
read_some(MutableBufferSequence const& buffers, error_code& ec)
{
    ...
    if(b.size() + 5 > b.max_size())
    {
        copy_ = static_cast<unsigned char>(
            n + 5 - b.max_size());
        std::copy(
            asio::buffers_begin(buffers) + n - copy_,
            asio::buffers_begin(buffers) + n,
            buf_);
        n = b.max_size() - 5;
    }
    buffers_suffix<beast::detail::buffers_ref<
