...
using ConstBufferSequence =
    BufferSequence<asio::const_buffer>;
using MutableBufferSequence =
    BufferSequence<asio::mutable_buffer>;

template<class B1, class... Bn>
struct is_all_const_buffer_sequence
    : std::integral_constant<bool,
        asio::is_const_buffer_sequence<B1>::value &&
...
struct is_all_const_buffer_sequence<B>
    : asio::is_const_buffer_sequence<B>
{
};
    ...
    using type = typename std::conditional<
        boost::is_convertible<std::tuple<Bn...>,
            typename repeat_tuple<sizeof...(Bn),
                asio::mutable_buffer>::type>::value,
                    asio::mutable_buffer,
                        asio::const_buffer>::type;
};
...
public:
    ...
    {
        return asio::buffer_sequence_begin(b_);
    }

    const_iterator
    end() const noexcept
    {
        return asio::buffer_sequence_end(b_);
    }
};
...
*/
#define BEAST_HANDLER_INIT(type, sig) \
    static_assert(beast::is_completion_handler< \
    ASIO_HANDLER_TYPE(type, sig), sig>::value, \
    "CompletionHandler signature requirements not met"); \
    asio::async_completion<type, sig> init{handler}
