...
/** A @b Stream with attached @b DynamicBuffer to buffer reads.
    ...
    `asio::buffered_readstream`. It is designed to facilitate
    the use of `asio::read_until`, and to allow buffers
    ...
    buffered version of `asio::ssl::stream::async_handshake`
    could make use of this wrapper.

    Uses:

    @li Transparently leave untouched input acquired in calls
      to `asio::read_until` behind for subsequent callers.
