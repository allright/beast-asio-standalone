...
public:
    ...
    operator*() const
    {
        ...
        return value_type{b.data(),
            (ba_->out_ == asio::buffer_sequence_end(ba_->bs_) ||
...
buffers_adapter(buffers_adapter&& other)
    : buffers_adapter(std::move(other),
        std::distance<iter_type>(asio::buffer_sequence_begin(other.bs_), other.begin_),
        std::distance<iter_type>(asio::buffer_sequence_begin(other.bs_), other.out_),
        std::distance<iter_type>(asio::buffer_sequence_begin(other.bs_), other.end_))
{
}
    ...
    buffers_adapter&
{
    auto const nbegin = std::distance<iter_type>(
        asio::buffer_sequence_begin(other.bs_),
            other.begin_);
    ...
    begin_ = std::next(asio::buffer_sequence_begin(bs_), nbegin);
    out_ =   std::next(asio::buffer_sequence_begin(bs_), nout);
    end_ =   std::next(asio::buffer_sequence_begin(bs_), nend);
...
buffers_adapter(MutableBufferSequence const& bs)
    : bs_(bs)
    , begin_(asio::buffer_sequence_begin(bs_))
    , out_  (asio::buffer_sequence_begin(bs_))
    , end_  (asio::buffer_sequence_begin(bs_))
    , max_size_(asio::buffer_size(bs_))
{
}
    ...
    mutable_buffers_type
{
    ...
    end_ = out_;
    if(end_ != asio::buffer_sequence_end(bs_))
    {
