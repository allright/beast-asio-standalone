...
asio::const_buffer get_next_chunk_body();
//]
...
//[http_snippet_18
    ...
    asio::write(sock, make_chunk(get_next_chunk_body()));
    asio::write(sock, make_chunk(get_next_chunk_body()));
    asio::write(sock, make_chunk(get_next_chunk_body()));

    // We are responsible for sending the last chunk:
    asio::write(sock, make_chunk_last());
//]
}
...
//[http_snippet_19
    ...
    asio::write(sock, make_chunk(get_next_chunk_body(), ext));
    ...
    asio::write(sock, make_chunk(get_next_chunk_body(), ext, std::allocator<char>{}));
    ...
    // Note: ext is moved
    asio::write(sock, make_chunk(get_next_chunk_body(), std::move(ext)));
//]
}
...
//[http_snippet_21
    ...
    asio::write(sock, make_chunk_last(trailer));
//]
}
...
//[http_snippet_22
    ...
    asio::write(sock, make_chunk_last(trailer, std::allocator<char>{}));
//]
}
...
//[http_snippet_23
    ...
    string_view ext =
        ...
        "Expires: never\r\n"
        "\r\n";
    asio::write(sock, make_chunk_last(asio::const_buffer{ext.data(), ext.size()}));
//]
}
...
//[http_snippet_24
    ...
    asio::write(sock, chunk_header{
        asio::buffer_size(cb1) +
        asio::buffer_size(cb2) +
        asio::buffer_size(cb3)});
    ...
    do
    {
        sr.next(ec,
            [&sr](error_code& ec, auto const& buffer)
            {
                ...
                sr.consume(asio::buffer_size(buffer));
            });
    }
    while(! ec && ! sr.is_done());
    if(! ec)
